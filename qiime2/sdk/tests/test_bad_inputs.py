...
class TestBadInputs(TestPluginBase):
    ...
    def test_primitive_param_not_valid_choice(self):
        pipeline_with_Choices = self.plugin.pipelines['failing_pipeline']
        int_sequence = Artifact.import_data(IntSequence1, [1, 2, 3])

        # passes primitive of correct type but invalid choice
        with self.assertRaisesRegex(
                TypeError, 'break_from.*invalidStr.*incompatible.*Str % Choi'):
            pipeline_with_Choices(int_sequence, 'invalidString')
